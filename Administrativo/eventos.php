...
<head>
	...
	<script type="text/javascript">
		function confirmar_eliminar(){
			return confirm("¿Seguro/a que deseas eliminar el evento?");
		}
	</script>
    <?php
		include('head.php');
                                    ...
                                    <div class="tab-content">
                                        <div id="inbox" class="tab-pane fade in animated zoomInDown custom-inbox-message shadow-reset active">
                                            <div class="mail-title inbox-bt-mg">
												<a href="gestContenido.php?opc=nuevo&tipo=evento" class="btn btn-primary btn-sm" style="background-color: green;width:150px;"><font color="white">+ Crear Evento</font></a>
                                                <h2>Eventos</h2>
                                                <div class="view-mail-action view-mail-ov-d-n">
                                                    <a href="eventos.php"><i class="fa fa-reply"></i> Recargar</a>
                                                ...
                                                <table id="table1" style="width:100%" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-show-toggle="true" data-resizable="true" data-cookie="true" data-cookie-id-table="saveId" data-show-export="true" data-click-to-select="true" data-toolbar="#toolbar1">
                                                    <thead>
                                                        <tr>
															...
															<th data-field="URL"><center>URL</center></th>
													...
													<?php
														$con->query("SELECT * FROM tp_evento ORDER BY tpc_validohasta_evento DESC;");
														if($con->num_rows() > 0){
															...
															<tbody>';
																while($con->next_record()){
																	...
																	echo '<tr>
																		<td><center><input type="checkbox" id="checkespecifico'.$contador.'" name="checktodos" value="'.$con->f("tpc_codigo_evento").'"></center></td>
																		<td><center><label>'.$con->f("tpc_nombre_evento").'</label></center></td>
																		<td><center><label>De '.$con->f("tpc_validodesde_evento").' A '.$con->f("tpc_validohasta_evento").'</label></center></td>
																		<td><center><label><a href="'.$con->f("tpc_archivo_evento").'" download>Descargar Archivo</label></center></td>
																		<td><center><label>'.$con->f("tpc_url_evento").'</label></center></td>
																		<td><center><label><a href="gestContenido.php?tpc_codigo_evento='.$con->f("tpc_codigo_evento").'&opc=editar&tipo=evento"><i class="fa fa-edit"></a></label></center></td>
																		<!--<td><center><label><a href="gestContenido.php?tpc_codigo_evento='.$con->f("tpc_codigo_evento").'&opc=activacion&tipo=evento">'.$activo.' '.$estados[$con->f("tpc_estado_documento")].'</a></label></center></td>-->
																		<td>
																			<center>
																				<form method="post" action="gestContenido.php" onSubmit="return confirmar_eliminar()">
																					<input type="hidden" name="tpc_codigo_evento" id="tpc_codigo_evento" value="'.$con->f("tpc_codigo_evento").'">
